class Game
	...
	def players_bet
		players.each do |player|
			if player.eliminated?
				puts "#{player.name_} passes. (Out of chips!)"
			else
				print "#{player.name_} can bet between 0 and #{max_bet(player)}: "
				bet = gets.to_i
				if bet < 0 || bet > max_bet(player)
					bet = 0
				end
				puts "#{player.name_} bet #{bet}"
				player.bet = bet
			end
		end
	end
	...
	def show_player_chips
		players.each do |player|
			if player.eliminated?
				puts "#{player.name_} has been eliminated"
			else
				puts "#{player.name_} has #{player.chips} chips"
			end
		end
	end
	...
	def game_over
		...
		puts "The winner is #{players.last.name_}"
	end
end